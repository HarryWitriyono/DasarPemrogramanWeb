...
<div class="container">
  <h2>Koreksi Rekord Pengguna</h2>
  <form method="post">
    <div class="form-group">
      <label for="email">Ketik kode pengguna yang dikoreksi:</label>
      <input type="text" class="form-control" id="email" placeholder="Ketik kode pengguna yang akan dikoreksi" name="namadikoreksi">
    </div>
...
if (isset($_POST['bSimpan'])) {
	...
	$NoHP=$_POST['NoHP'];
	$koneksi=new mysqli("localhost","root","","tokoumb");
	$sql="UPDATE `pengguna` SET `NamaPengguna` = '$NamaPengguna', `Password` = '$Password', `Alamat` = '$Alamat', `NoHP` = '$NoHP' WHERE `pengguna`.`KodePengguna` = '$KodePengguna';";
	$q=$koneksi->query($sql);
	if ($q) {
